...
enum { SchemeNorm, SchemeInv, SchemeSel, SchemeHov, SchemeHid }; /* color schemes */
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
        ...
        /* 2px right padding */
		else if (ev->x > selmon->ww - (int)TEXTW(estextr) + lrpad - 2)
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* extra status is only drawn on selected monitor */
		...
		/* Draw awesomebar on extra bar */
        // maybe put a config option to disable this if needed
		if ((w = m->ww - etwr) > bh && n > 0) {
			int remainder = w % n;
...
updatebars(void)
{
	...
	for (m = mons; m; m = m->next) {
        ...
        if (!m->extrabarwin) {
            ...
            XSelectInput(dpy, m->extrabarwin, ButtonPressMask|PointerMotionMask);
